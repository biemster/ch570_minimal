...
# Find the device
device = usb.core.find(idVendor=CH_USB_VENDOR_ID2, idProduct=CH_USB_PRODUCT_ID)
...
# chip detect, should be [161, 0, 2, 0, 112, 19]
device.write(CH_USB_EP_OUT, CH_STR_CHIP_DETECT)
print(device.read(CH_USB_EP_IN, CH_USB_PACKET_SIZE, CH_USB_TIMEOUT))

# config read, EVT default is [167, 0, 26, 0, 31, 0, 255, 255, 255, 255, 255, 255, 255, 255, 72, 0, 255, 64, 0, 2, 3, 0, 47, 68, 33, 98, 50, 220, 130, 130]
device.write(CH_USB_EP_OUT, CH_STR_CONFIG_READ)
print(device.read(CH_USB_EP_IN, CH_USB_PACKET_SIZE, CH_USB_TIMEOUT))

# Write good config, like the WeAct modules came with
# device.write(CH_USB_EP_OUT, b'\xa8\x0e\x00\x07\x00' + bytes([255, 255, 255, 255, 255, 255, 255, 255, 213 if DEBUG_ENABLE else 72, 0, 255, 64]))
# print(device.read(CH_USB_EP_IN, CH_USB_PACKET_SIZE, CH_USB_TIMEOUT))
